import "./Produit.css";
import Modal from "./ModalOper";
import React, { useState, useEffect } from "react";
import axios from "axios";

function Operation() {
  const Pr_Object = "Operations";
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:8001/get_oper")
      .then((response) => {
  ...
  const filtered = List.filter(
    (element) =>
      ...
      element.duration.toLowerCase().includes(searchInput.toLowerCase())
  );
  ...
  const handleSubmit = (e) => {
    ...
    const requiredFields = ["operation", "duration"];
    ...
    axios
      .post("http://localhost:8001/check_oper", [mappedArray[0]])
      .then((response) => {
        if (response.data.exists) {
          alert("Operation exists déja!");
        } else {
          axios
            .post("http://localhost:8001/insert_oper", [mappedArray])
            .then((response) => {
              // Refresh the product list after inserting a new product
              axios.get("http://localhost:8001/get_oper").then((response) => {
                setList(response.data);
              });
          ...
          alert("Operation insérée avec succes");
        }
      })
      .catch((error) => {
  ...
  const handleDelete = () => {
    ...
    if (isConfirmed) {
      axios
        .post("http://localhost:8001/delete_oper", selected)
        .then((response) => {
          if (response.data === "Success") {
            const updatedList = List.filter(
              (element) => !selected.includes(element.op_id)
            );
          ...
          <table className="tableproduct">
            ...
            <tbody>
              {filtered.map((element) => (
                <tr key={element.op_id}>
                  <td>{element.operation}</td>
                  <td>{element.duration}</td>
                  <td className="checkbox-cell">
                    <input
                      type="checkbox"
                      checked={selected.includes(element.op_id)}
                      onChange={() => handleCheckboxChange(element.op_id)}
                    />
                  </td>
                </tr>
              ))}
